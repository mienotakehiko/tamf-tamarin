...
if [ -z "${SKIP_KEMTLS:-}" ]; then
  ...
  if [ -d external/kemtls-tls13tamarin ]; then
    ...
    log "  external/kemtls-tls13tamarin not present; see docs/getting-external-artifacts.md"
  fi
else
...
if [ -z "${SKIP_XREPO:-}" ]; then
  ...
  for target in tls13tamarin 5g-aka; do
    if [ -d "external/${target}" ]; then
      ...
      log "  external/${target} not present; see docs/getting-external-artifacts.md"
    fi
  done
else
