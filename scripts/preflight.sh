...
if command -v tamarin-prover >/dev/null 2>&1; then
    ...
    if echo "$TVER" | grep -qE "1\.(10|12)\.0"; then
        ...
        check "tamarin-prover 1.10.0 or 1.12.0"  WARN  "(found $TVER, rev082 baseline used 1.12.0)"
    fi
else
...
if [ "$MEM_TOTAL_KB" -ge 7340032 ]; then   # 7 GiB
    ...
    check "physical memory >= 7 GiB" WARN "($MEM_TOTAL_GIB GiB found; A_raw peaks ~6.2 GiB in rev082 baseline, OOM risk)"
else
