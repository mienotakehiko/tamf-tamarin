...
(older builds) we fall back to the last line before "Proving lemmas".
When both markers are missing we mark derivcheck_s as NA.
...
# Phase markers we look for in stderr. Order matters.
PHASE_MARKERS = [
...
def analyse_stderr(stderr_path: Path) -> dict[str, int | None]:
    ...
    alone. We therefore use two mechanical signals:
...
def main() -> int:
    ...
    for stderr_path in sorted(args.runs_dir.glob("*.stderr")):
        ...
        # printed, note this by leaving derivcheck_s as NA (we do not fabricate
