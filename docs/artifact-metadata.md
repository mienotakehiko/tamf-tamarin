# Robustness Battery — Artifact Metadata

## Model file provenance

All four `.spthy` models are byte-identical copies from the rev082 evidence kit
(`rev082-evidence-kit.tar.gz`, distributed alongside the STTT initial submission).
...
(§3.4 in the manuscript, `verify_correspondence.py` in the rev082 kit)
reports `overall = PASS` for A↔C and B↔D with `diff = 0` on all 11 rules.

## Reproducing the rev082 command line

The exact command line used for the rev082 timeout-curve baseline was:
...
Every run uses `+RTS -N2 -RTS`. This matches rev082. No memory cap
...
## What "same as rev082" means

The Battery is designed to be a **strict superset** of the rev082 timeout-curve
data at the 300 s budget: the (A, `s`) and (C, `s`) cells reproduce rev082
...
from the rev082 medians (71.68 s for C, 300 s TIMEOUT for A), the
